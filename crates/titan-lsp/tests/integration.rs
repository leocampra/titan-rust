...
fn completar_em_posicao_de_expressao_lista_escopo_builtins_e_keywords() {
    ...
    let labels = completion_labels(&response);
    // As oito últimas são as keywords que a Fase 5 acrescentou (sete na
    // T59, mais `with` na T75): chegam ao autocomplete só por entrarem em
    // `KEYWORDS`, sem trabalho no LSP.
    for esperado in [
        "x", "args", "print", "if", "while", "function", "enum", "match", "continue", "repeat",
        "until", "in", "foreign", "with",
    ] {
        assert!(
            labels.contains(&esperado.to_string()),
